...
from . import caffe_option
from . import prompt
from digits import device_query
from digits.utils import parse_version
from digits.utils.errors import UnsupportedPlatformError

class MultipleCaffeOption(config_option.FrameworkOption):

    @staticmethod
    def config_file_key():
        return 'multiple_caffe'

    @classmethod
    def prompt_title(cls):
        return 'Multiple Caffe'

    @classmethod
    def prompt_message(cls):
        return 'Do you want to use multiple Caffe installations?'
    ...
    def validate(cls, value):
        ...
        print "multiple validating"
        if not value:
            return value

        if value not in ('1', '0'):
            raise config_option.BadValue('set multiple caffe to 0 or 1')
